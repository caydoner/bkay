...
tabs_inner = tabs_match.group(1).replace('className={`flex items-center', 'className={`flex items-center min-w-max pb-4 border-b-2 font-bold transition-all text-sm').replace('py-4 border-b-2', '') # Clean up tab padding
...
# Replace everything from `return (` down to `div className="flex-1... {activeTab === 'general'`
...
# Also fix the closing tag at the end of the file
new_code = new_code.replace('                </div>\n            </div>\n        </>\n    );\n}', '            </div>\n        </div>\n    );\n}')

# Now process the contents to give them spatial-first styles
# General Tab
...
# Map Tab (Extract tools/grid generation into floating left pane)
...
new_code = re.sub(r"\{activeTab === 'table' && \(\s*<div className=\"max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300\">\s*<div className=\"flex justify-between items-center glass-panel border-white/5 p-6 rounded-3xl shadow-\[0_0_15px_rgba\(0,0,0,0\.5\)\] border border-white/10\">", table_style_replace, new_code)
...
new_code = re.sub(r"\{activeTab === 'forms' && \(\s*<div className=\"max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300\">\s*<div className=\"flex justify-between items-center mb-6\">", form_style_replace, new_code)
...
new_code = re.sub(r"\{activeTab === 'stakeholders' && \(\s*<div className=\"max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300\">\s*<div className=\"glass-panel border-white/5 p-6 rounded-3xl shadow-\[0_0_15px_rgba\(0,0,0,0\.5\)\] border border-white/10 flex justify-between items-center\">", stakeholders_style_replace, new_code)
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_code)
print("Structural refactoring applied!")
